...
const FeedbackPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:3002/api/feedback/feedbackp",
        {
          method: "POST",
          body: formData,
        }
  ...
  return (
    <div className="flex justify-center items-center dark:bg-gray-900 min-h-screen bg-gradient-to-r from-green-500 to-lime-200 p-28">
      ...
      >
        ...
        <p className="text-gray-600 text-center mb-4">
          Let us know how we can assist you with your gardening needs.
        </p>
        <div className="space-y-4">
          ...
          <div className="border border-dashed border-gray-400 p-4 rounded-lg text-center cursor-pointer">
            ...
            >
              <FaImage className="text-green-500 text-2xl mr-2" /> Upload an
              Image (Optional)
            </label>
            <input
              id="image-upload"
            ...
            {image && (
              <img
                ...
                alt="Uploaded"
                className="mt-4 max-h-40 mx-auto rounded-lg shadow-md"
              />
            )}
          </div>
        </div>
        <button
          className={`w-full mt-4 py-3 text-white rounded-lg transition-all ${
            isValid
        ...
        {submitted && (
          ...
          >
            <FaCheckCircle className="mr-2 text-green-500" /> Your feedback has
            been submitted. We will assist you shortly!
          </motion.div>
        )}
      </motion.div>
    </div>
